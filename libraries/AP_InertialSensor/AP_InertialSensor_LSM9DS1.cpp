...
void AP_InertialSensor_LSM9DS1::_fifo_reset()
{
    ...
    _dev->write_register(LSM9DS1XG_CTRL_REG9,LSM9DS1XG_CTRL_REG9_FIFO_EN);
...
{
    if (!_imu.register_gyro(_gyro_instance, 238, _dev->get_bus_id_devtype(DEVTYPE_GYR_LSM9DS1)) ||
        !_imu.register_accel(_accel_instance, 238, _dev->get_bus_id_devtype(DEVTYPE_ACC_LSM9DS1))) {
        return;
    }
